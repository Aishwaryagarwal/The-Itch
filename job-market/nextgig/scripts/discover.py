    ...
    for p in paths:
        with open(p, newline="", encoding="utf-8", errors="ignore") as f:
            for row in csv.DictReader(f):
                ...
                if title_filter and soc == "15-1252" and not TITLE_HINT.search(title):
                    continue      # generic SWE filings without a data signal
...
def score(e, obscure_bonus=True):
    ...
    if obscure_bonus and norm(e["name"]) not in WELL_KNOWN:
        s += 20
...
def main():
    ...
    for k, e in emp.items():
        if not (args.min_filings <= e["filings"] <= args.max_filings):
            continue
        if args.obscure and k in WELL_KNOWN:
            continue
        if states:
            here = {s.split(", ")[-1] for s in e["sites"] if ", " in s} | {e["st"].upper()}
            if not (here & states):
                continue
